...
async function initializerCall(): Promise<number> {
  const value: number = combine(
    "init call",
    wrap("init wrap", await Promise.resolve(mark("init left", 1))),
...
async function terminalCall(): Promise<number> {
  return combine(
    "return call",
    wrap("return wrap", await Promise.resolve(mark("return left", 10))),
...
async function discardCall(): Promise<void> {
  consume(
    "stmt call",
    wrap("stmt wrap", await Promise.resolve(mark("stmt left", 100))),
