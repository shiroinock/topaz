...
- [ ] **2.2b stdlib aliases** — `std/path` または `std/fs` から loader allowlist と codegen alias を小さく実装する。既存 `node:*` import は self-host/compiler 互換として残し、`src/*.ts` の置き換えは alias が green になってから判断する。
- [ ] **2.2c process stdlib design** — `process.argv` / `process.exit` / `process.stdout.write` / `process.stderr.write` / `console.error` の synthetic/global support を public `std/process` API としてどう命名するかを ADR で決める。
