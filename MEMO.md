...
- [ ] **2.4b bigint frontend/type skeleton** — `bigint` 型注釈、bigint literal AST node または等価表現、decimal `123n` literal を受理する。最初の pass で hex/bin/octal literal を入れない場合は、非 decimal bigint literal を clear diagnostic で reject する。
- [ ] **2.4c bigint arithmetic/comparison runtime** — binary `+` / `-` / `*`、unary `-`、`<` / `<=` / `>` / `>=`、`===` / `!==`、`console.log` と template literal stringification を最初の runtime/codegen surface とする。`/` / `%`、bitwise / shifts、`BigInt()`、`.toString()`、hex/bin/octal literal、Array/Map/Set 内 bigint、hashing、parse/format performance は後続設計へ回す。
- [ ] **2.4d regexp design** — regexp literal / `RegExp` / match API / runtime 表現を個別 ADR で設計する。
