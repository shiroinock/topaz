...
- [ ] **1.5 残り** — 以下を 1.5-N サブフェーズに分けて進める。順序は self-hosting に必要なものから逆算で入れ替える。詳細な棚卸し根拠は `docs/archive/self-hosting-inventory.md`。
  ...
  - [x] **1.5-6 prep** — self-hosting 着手前の地ならし、38 サブステップ完了(access modifier no-op / void return / field initializer + auto-ctor / type alias / object literal & anonymous class / object destructuring / optional parameter & field / Array<dunion> / module-level const hoist / string method (charCodeAt + slice) / dunion-context object literal / String.fromCharCode / node:fs.readFileSync / recursive type alias (SCC + 2-phase fill) / `T | undefined` for T = dunion / global parseInt + parseFloat / equality without redundant outer parens / dunion 共通フィールドの narrow なし read / compound condition narrowing (`&&` / `||` の右オペランド) / compound 条件の early-exit carry narrowing (De Morgan 分解) / dunion 初期化子 narrowing (`const x: U = <variant>`) / object literal property shorthand (`{ x }` → `{ x: x }`) / dunion → より広い dunion への widening (部分集合 variant) / condition 省略形 `for (;;)` 無限ループ / conditional (ternary) `cond ? a : b`(branch 別 narrowing + 共通 target 寄せ)/ `dunion | undefined` を expected とする object literal(undefined を剥がして inner で emit → applyCoercion で widen)/ IIFE `(() => {...})()` の contextual return type 推論(expected を arrow 戻り型として供給)/ closure 越えの dunion narrowing 伝播(capture 解析の `lookupAcrossBarrier` を narrowing-aware 化、ternary arm / if 内で構築した IIFE が外側 dunion の narrowing を捕捉)/ `node:fs.existsSync(path): boolean`(readFileSync と同じ call-site 限定 shortcut、runtime は `access(F_OK)`)/ `node:path.dirname` + `resolve`(POSIX path.posix の C 移植、resolve は variadic + getcwd フォールバック + `.`/`..` 正規化、loader の module 解決依存)/ `node:fs.writeFileSync(path, content): void`(2 引数 string、encoding implicit utf8、fopen "wb"+fwrite、void 戻りで value 文脈は CodegenError、cli.ts の writeFileSync 呼び出し向け)/ `node:fs.mkdirSync(path, { recursive: true }): void`(recursive-only に固定、第 2 引数は syntactic object literal で `recursive: true` のみ受理、runtime は segment 単位 mkdir + EEXIST ignore、cli.ts の mkdirSync 呼び出し向け)/ `node:path.basename(path, ext?): string`(POSIX path.posix.basename の C 移植、arity 1/2 で `topaz_path_basename` / `topaz_path_basename_ext` に dispatch、trailing slash 剥がし + `suffix === path` 早期 "" + RTL ext-index scan、cli.ts の basename 呼び出し向け)/ `node:path.extname(path): string`(POSIX path.posix.extname の C 移植、RTL scan で `startDot` / `startPart` / `preDotState` を 1 パス追跡、`.` / `..` / leading-dot-only segment は `""` に倒して Node と bit-for-bit 一致、cli.ts の extname 呼び出し向け)/ `node:path.join(...segments): string`(POSIX path.posix.join の C 移植、variadic + 空セグメント skip + arity 0 で `.` を返す Node 仕様、leading `/` / trailing `/` を normalize 前後で保持、cli.ts:71 の `join(dirname, basename)` 呼び出し向け)/ `node:child_process.execFileSync(cmd, args, { stdio: "inherit" }): void`(stdio inherit 固定の call-site shortcut、第 3 引数は `mkdirSync` と同じ syntactic object literal、runtime は fork + execvp + waitpid で非 0 exit / signal は abort、cli.ts:88 の cc 起動向け)/ `node:url.fileURLToPath(url): string` + `import.meta.url`(POSIX 限定で `file://` prefix + 空 / `localhost` host + percent-decode、`import.meta.url` は MetaProperty 経路を `topaz_runtime_module_url()` 1 本に絞り `__APPLE__` は `_NSGetExecutablePath` / `__linux__` は `readlink("/proc/self/exe")` + realpath + `file://` prefix を static[4096] にキャッシュ、`import.meta.X` の他 property / 裸 `import.meta` / `new.target` は reject、cli.ts:85 の `dirname(fileURLToPath(import.meta.url))` 向け)/ `process.argv` + `process.exit(code?)` + `process.{stdout,stderr}.write(s)` + `console.error`(`process` / `console` は synthetic namespace で call-site / value-site 限定認識、`process.argv` は毎回 fresh な `topaz_array_string`(要素は OS argv バイトを alias、Node の `[node, script, ...]` と違い `[exe, ...]` で 1 つ少ない)、main を `main(int __topaz_argc, char **__topaz_argv)` 化して unconditional に `topaz_runtime_init_argv` で stash、`process.exit` は never で value 文脈 reject + arity 0/1、stream write は newline 無し void、console.error は console.log の scalar lowering を stderr に流すだけ、cli.ts の die / usage / lex/parse-only dump 向け)))。決定ログは #1〜#15 が `docs/archive/implementation-log.md`、#16 以降が `docs/adr/`(parseInt/parseFloat = `0003`、equality = `0004`、dunion 共通フィールド = `0005`、compound 右オペランド narrowing = `0006`、compound carry narrowing = `0007`、dunion 初期化子 narrowing = `0008`、object literal shorthand = `0009`、dunion widening = `0010`、`for (;;)` = `0011`、ternary = `0012`、dunion-optional object literal = `0013`、try body 内 return = `0014`、IIFE contextual return = `0015`、closure narrowing 伝播 = `0016`、node:fs.existsSync = `0017`、node:path.dirname/resolve = `0018`、node:fs.writeFileSync = `0019`、node:fs.mkdirSync = `0020`、node:path.basename = `0021`、node:path.extname = `0022`、node:path.join = `0023`、node:child_process.execFileSync = `0024`、node:url.fileURLToPath + import.meta.url = `0025`、process/console builtin = `0026`)。**Topaz-subset rewrite 済みファイル(`ast.ts` / `lexer.ts` / `topaz_parser.ts`)が全て `--emit-c-only` を通り `cc -c` でオブジェクト化成功(残る `-Wreturn-type` 警告は全 path return/throw の関数特性)** — prep phase の parser 経路は解消。残る `codegen.ts` / `loader.ts` / `cli.ts` / `convert_from_tsc.ts` / `parser_check.ts` / `parser.ts` は namespace import / `node:child_process` / tsc API 依存で、1.5-6e〜h の Topaz-subset rewrite 待ち(prep の単発 lowering blocker ではない)。loader.ts は `node:fs.existsSync` + `node:path.dirname`/`resolve` の解禁で stdlib import を抜け、残る blocker は tsc API の namespace import(1.5-6g)。`cli.ts` の `node:fs` / `node:path` / `node:child_process` / `node:url` import 行はこれで全 named import が通り、`process.argv` / `process.exit` / `process.{stdout,stderr}.write` / `console.error`(`die` / usage / lex/parse-only dump)も prep #26 で解禁済み。残る blocker は loader が弾く `node:util.parseArgs` 1 specifier のみ。**設計判断(ADR `0026` Consequences)**: parseArgs は builtin 化せず(per-site anon class 合成 + hyphenated bracket access + truthy/falsy on dunion が連鎖する大物を cli.ts 1 箇所のためだけに作るのは minimalism に反する)、1.5-6h の cli.ts Topaz 化で argv 手書きパースに置き換えて消す方針に確定。
  ...
  - [ ] **1.5-X (オプション)** — `finally` 句および try body 内 return / break / continue 解禁。cleanup の lowering(`__cleanup__` attribute or 手書きの dispatch tree)とセット。self-hosting で実際に欲しくなった時点で着手、不要なら Phase 2 に持ち越し。**try body 内 `return` は着手済み**(`liveTryFrames` カウンタで脱出時に `topaz_try_pop()` を出す、決定ログ `0014`)。残りは `finally` / try body 内 break / continue。
