...
- [ ] **1.5 残り** — 以下を 1.5-N サブフェーズに分けて進める。順序は self-hosting に必要なものから逆算で入れ替える。詳細な棚卸し根拠は `docs/archive/self-hosting-inventory.md`。
  ...
  - [ ] **1.5-3.5 (新設)** — **syntactic sugar 集中投入**。`src/codegen.ts` の棚卸しで 1.5-3 単独では self-hosting に届かないことが判明したため、別サブフェーズに切る。中身: `for-of`(58 箇所、**Array<T> は 1.5-3.5b、Set / Map.values() / Map.keys() は 1.5-3.5g-mapset で着地**、destructuring 必須の `.entries()` は destructuring サブステップ待ち)、arrow function + closure キャプチャ(22 箇所、`Array.map` のため、**1.5-3.5e で着地**)、template literal(280 箇所、**1.5-3.5a で着地**)、destructuring、optional chaining(`?.`、**1.5-3.5d で着地**)、nullish coalescing(`??`、**1.5-3.5c で着地**)、non-null assertion(`!`、**1.5-3.5c で着地**)、spread(`...x`、関数 args 渡しのみ)、`Array.map` / `.filter` / `.join` / `.includes` / `.slice`(**1.5-3.5f で着地**)、`Map.values()` / `Set.values()`(**1.5-3.5g-mapset で着地**、Iterator interface 経由の bare 利用は **1.5-3.5g-iterator** サブステップ待ち)。**1.5-4 (ヒープ管理)と一緒に検討する**: template literal を `+` に lowering すると毎回 malloc が走るため、arena の前にこれを入れると leak が劇的に増える(→ arena 着地後の 1.5-3.5a で着手して回避)。
    ...
    - [ ] **1.5-3.5g-iterator** — 最小 `Iterator<T>` interface + bare `.values()` / `.keys()` の解禁。**狙い**: 現状 `.values()` / `.keys()` の戻り値は for-of の RHS でしか使えない「単独で表現できない値」、これを `Iterator<T>` として first-class に昇格して `const vs = m.values()` / 関数渡し / `let iter` / for-of 経由消費を全て同一型で扱えるようにする。**TODO**:
        - [ ] **C 表現の決定**。3 案検討: (a) **state struct + 関数ポインタ fat pointer** = `topaz_iter_<T> = { void *state; T (*next)(void *state, bool *done); }`、state を 1.5-3.5e の env と同じく arena alloc、`done` は out-param で yield 終了を伝える(`T | undefined` を返すと scalar T で undefined slot を持てない問題が出る)、(b) **trait interface** = `Iterator<T>` を topaz の interface 機構に乗せて `hasNext(): boolean` + `next(): T` の vtable dispatch、per-(I, source) wrapper を生成、(c) **closure-based** = arrow を返す関数で state を capture、`next` は arrow 値の `.fn` を呼ぶだけ。**第 1 候補は (a)**: 1.5-3.5e の fn 値表現と構造が揃う、closure-based より `done` flag が綺麗、trait より per-source 監視コードが少ない。
        - [ ] **`runtime/runtime.h` に `topaz_iter_<T>` typedef + 各 source(Map / Set)用の state struct + `_next` 関数を追加**。Map.values() なら `topaz_iter_state_map_<K>_<V>_values = { topaz_map_<K>_<V> *src; size_t idx; }` + `static V topaz_iter_map_<K>_<V>_values_next(void *state, bool *done)` で 1.5-3.5g-mapset の cap-based walk + OCCUPIED skip を再利用。per-(source kind, T) monomorph を codegen が登録、`recordIteratorMonomorph(sourceKind, T)` を新設。
        - [ ] **bare 利用の解禁**。`emitMapMethodCall` / `emitSetMethodCall` / `inferType` の 3 箇所の reject を解除、戻り値型を `{ kind: "iter", elem: T }` の新 `TopazType` variant に。`const vs = m.values()` で `topaz_iter_<V> vs = ({ ... state alloc + iter compound literal ... });` に lower、`emitWithExpected` 経路で関数引数 / 戻り値の Iterator<T> coercion を確立。
        - [ ] **for-of の RHS に `Iterator<T>` を許す**(`emitForOfStatement` に 5 番目の dispatch 経路)。iter binding は `while` 形式に lower: `{ topaz_iter_<T> __it = recv; while (true) { bool __done = false; <T> __v = __it.next(__it.state, &__done); if (__done) break; <T> bindName = __v; <body>; } }`。`continue` / `break` は while loop に自然に乗る。既存の Map.values() / Set RHS の直接 walk(1.5-3.5g-mapset)とは経路を分離(special form は引き続き直接 slot walk、bare iter binding 経由のみ while 経路)、performance regression を避ける。
        - [ ] **回帰**: `iterator_basic.ts`(`const vs = m.values()` を別変数 bind / 関数引数渡し / 関数 return / for-of で消費 / 二重消費(2 回目は empty)/ Set.values() と Map.keys() の混在)、`iterator_for_of.ts`(Iterator<V> を for-of の RHS で消費、`continue` / `break` / nested)、fail 1〜2 本(現状の `map_values_outside_for_of_fail.ts` を positive 化 + 削除、新たに「`.entries()` を bare 利用」や「`Array<T>` を直接 iter として扱う」を reject する負例を追加)。
        - [ ] **scope 外(別 step に切り出し)**: `.entries()` は引き続き reject(destructuring 必須、destructuring サブステップで着地)。`Iterator<T>.map` / `.filter` / `.toArray()` などの iter 自身のメソッドは持たない(必要なら for-of で書く)。`Symbol.iterator` 等の JS の protocol は持ち込まない(Topaz 固有 type として扱う)。lazy evaluation の Array methods(`.map` を iter chain 化する設計)も着手しない、Array 系は引き続き eager。
