...
- [x] **4.35 v0.1.3 release state handoff** — `docs/releases/v0.1.3-release-state-handoff.md` に local gates が green でも final `v0.1.3` tag が current HEAD へ peel しない場合の非変更 handoff を記録し、release skill / pre-v0.2 checkpoint から参照するようにした。通常 `pnpm test` は handoff doc の gate command、tag/HEAD 比較、stale final tag 停止文言、allowed human-owned next action、release skill linkage、pre-v0.2 checkpoint linkage を静的に確認する。tag 作成/削除/移動/push、GitHub Release 編集/公開、release artifact/runtime/CLI/manifest/doctor/check/explain/permission behavior は変更しない。決定ログは `docs/adr/0454-v0-1-3-release-state-handoff.md`。
- [x] **4.36 v0.2.0 RC readiness checklist** — `docs/releases/v0.2.0-rc-readiness.md` に current guidance CLI surface から始める non-mutating RC readiness checklist を置き、release skill / pre-v0.2 checkpoint から参照するようにした。通常 `pnpm test` は checklist の local gate、`v0.2.0-rc.1` tag/HEAD 比較、downloaded artifact guidance smoke、future-out-of-scope 境界、linkage を静的に確認する。tag 作成/削除/移動/push、GitHub Release 編集/公開、CLI behavior、manifest schema、permission enforcement、runtime sandboxing、runtime/prelude/header、artifact 名、checksum format は変更しない。決定ログは `docs/adr/0455-v0-2-0-rc-readiness.md`。
- [x] **4.37 v0.2.0 release notes draft** — `docs/releases/v0.2.0.md` に capability / manifest guidance release track の final release notes draft を置き、release skill から `gh release edit v0.2.0 --notes-file docs/releases/v0.2.0.md` で適用できるようにした。通常 `pnpm test` は notes の section、checksum / guidance artifact smoke、future work 境界、workflow placeholder 不在、release skill linkage を静的に確認する。tag 作成/削除/移動/push、GitHub Release 編集/公開、CLI behavior、manifest schema、compile-time policy enforcement、runtime sandboxing、schema expansion、richer policy discovery、runtime/prelude/header、artifact 名、checksum format は変更しない。決定ログは `docs/adr/0456-v0-2-0-release-notes.md`。
- [x] **4.38 runtime substrate detail report** — `check-runtime-substrate --details` で残存 runtime substrate symbol ごとの category / migration lane / next guidance と `runtime/runtime.h` location を deterministic に表示し、通常 `pnpm test` は代表的な C ABI type / raw memory / container monomorph / libc-libm / host ABI / exception / string-buffer intrinsic / bigint-limb intrinsic の detail fragment を静的に確認する。runtime/header/prelude/generated runtime/release behavior は変更しない。決定ログは `docs/adr/0457-runtime-substrate-detail-report.md`。
- [x] **4.39 string Map/Set equality prelude bridge** — `topaz_string_eq` を Map/Set macro 用 C ABI bridge として残しつつ、実装を runtime prelude の `__topaz_string_eq` へ委譲して string key equality algorithm の TS 側 single source を固定した。container monomorph lane count / runtime behavior / generated container representation は変更しない。決定ログは `docs/adr/0458-string-eq-container-prelude-bridge.md`。
- [x] **4.40 boolean Map/Set helper prelude bridge** — `topaz_hash_boolean` / `topaz_key_eq_boolean` を Map/Set macro 用 C ABI bridge として残しつつ、実装を runtime prelude の `__topaz_boolean_hash` / `__topaz_boolean_key_eq` へ委譲して boolean key hash/equality algorithm の TS 側 ownership を固定した。`0/1` hash のみを対象にし、number/string/pointer hashing、container representation、lane count、runtime behavior は変更しない。決定ログは `docs/adr/0459-boolean-container-prelude-bridge.md`。
- [x] **4.41 number Map/Set equality prelude bridge** — `topaz_key_eq_number` を Map/Set macro 用 C ABI bridge として残しつつ、SameValueZero の equality decision を runtime prelude の `__topaz_number_key_eq` へ委譲した。`topaz_hash_number` の `uint64_t` bit hashing、container representation、lane count、runtime behavior は変更しない。決定ログは `docs/adr/0460-number-container-equality-prelude-bridge.md`。
- [x] **4.42 residual container hash substrate** — Phase 4.39-4.41 の bridge 化後に残る `topaz_hash_number` / `topaz_hash_string` / `topaz_hash_pointer` を residual C substrate として pin し、`uint64_t` / `size_t` / NaN / `-0`、FNV-1a byte hashing / unsigned overflow / hash-order risk、pointer-bit reference identity を checker details と smoke で固定した。runtime behavior / generated container representation / lane count は変更しない。決定ログは `docs/adr/0461-residual-container-hash-substrate.md`。
- [x] **4.43 post-4.42 release readiness sync** — v0.2 release-readiness docs と static smoke contract に Phase 4.39-4.42 後の runtime boundary を反映し、string/boolean/number equality bridge と residual number/string/pointer hash substrate の split を `pnpm run check:runtime-prelude` / `pnpm run check:runtime-header` / `pnpm run check:runtime-substrate -- --details` の release gate として明記した。runtime behavior / generated runtime / codegen / release artifact / tag / GitHub Release state は変更しない。決定ログは `docs/adr/0462-post-4-42-release-readiness-sync.md`。
- [x] **4.44 release build runtime substrate detail gate** — `pnpm run build:release` の local release artifact builder でも `pnpm run check:runtime-substrate -- --details` を self-host/copy/checksum 前に実行し、release skill preflight と通常 `pnpm test` の静的 contract で runtime prelude/header/substrate detail gate を固定した。runtime behavior / generated runtime / codegen / release artifact / tag / GitHub Release state は変更しない。決定ログは `docs/adr/0463-release-build-runtime-substrate-detail-gate.md`。
...
- async/await compatibility roadmap: `Promise<T>` type frontier and
  `Promise.resolve` / contextual `Promise.reject` value allocation are complete;
  next stages are scheduler continuations, async function lowering,
