...
- [ ] **1.5 残り** — 以下を 1.5-N サブフェーズに分けて進める。順序は self-hosting に必要なものから逆算で入れ替える。詳細な棚卸し根拠は `docs/archive/self-hosting-inventory.md`。
  ...
  - [ ] **1.5-3.5 (新設)** — **syntactic sugar 集中投入**。`src/codegen.ts` の棚卸しで 1.5-3 単独では self-hosting に届かないことが判明したため、別サブフェーズに切る。中身: `for-of`(58 箇所、**Array<T> は 1.5-3.5b、Set / Map.values() / Map.keys() は 1.5-3.5g-mapset で着地**、destructuring 必須の `.entries()` は destructuring サブステップ待ち)、arrow function + closure キャプチャ(22 箇所、`Array.map` のため、**1.5-3.5e で着地**)、template literal(280 箇所、**1.5-3.5a で着地**)、destructuring、optional chaining(`?.`、**1.5-3.5d で着地**)、nullish coalescing(`??`、**1.5-3.5c で着地**)、non-null assertion(`!`、**1.5-3.5c で着地**)、spread(`...x`、関数 args 渡しのみ)、`Array.map` / `.filter` / `.join` / `.includes` / `.slice`(**1.5-3.5f で着地**)、`Map.values()` / `Set.values()`(**1.5-3.5g-mapset で着地**、Iterator interface 経由の bare 利用は **1.5-3.5g-iterator** サブステップ待ち)。**1.5-4 (ヒープ管理)と一緒に検討する**: template literal を `+` に lowering すると毎回 malloc が走るため、arena の前にこれを入れると leak が劇的に増える(→ arena 着地後の 1.5-3.5a で着手して回避)。
    ...
    - [ ] **1.5-3.5g-array-fn** — `Array<fn>` storage の解禁。**狙い**: 1.5-3.5e で arrow を fat pointer fn 値として表現できるようになったが、container には詰められない状態(`arrow_in_container_fail.ts` で reject)。self-hosting で AST visitor の callback 列を `Array<(node) => void>` で持つユースケース、および `.map` の callback として fn を返す arrow(`xs.map((x) => (n) => n + x)` のような curry)を Array に貯めるパターンが想定されるため、Array 側を解禁する。**TODO**:
        - [ ] **`recordArrayMonomorph(t)` で elem が fn の場合の reject を解除**。`TOPAZ_ARRAY_DEFINE(fn_a<N>__..., topaz_fn_a<N>__...)` を吐く順序を確認(fn typedef slot が `containerMonomorphSlot` より前にあるので OK のはず、要 verify)。slot 順序が逆だと typedef 未定義で C コンパイル時に落ちるので、`emit` の slot 配置を改めて整理する。
        - [ ] **`arr.push(arrow)` / `arr[i] = arrow` / `arr[i].fn(arr[i].env, args)` の各サイトで dispatch を確認**。基本は既存 Array 経路と 1.5-3.5e の fn 値 dispatch の組み合わせで動くはず、`emitCall` の identifier 分岐の fn 値 fallback が `arr[i]` のような expression callee でも発火するか要 verify。expression callee の fn dispatch 経路が無ければ `emitFnValueCall` の入口を拡張する。
        - [ ] **`emitWithExpected` での fn 型 coercion**。Array<fn> 要素への代入で expected が fn 型のとき、(a) 注釈無し arrow に contextual typing を流す、(b) fn 型注釈付き arrow / fn-typed identifier を EXACT 一致 check、を確認(変数初期化の経路と一貫させる)。`arr.push((n) => n + 1)` で arrow の param 注釈が無くても通るようにする。
        - [ ] **`Array.map` callback が fn を返す経路の解禁**。1.5-3.5f-map の callback return 型 check で「fn 型は reject」を解除、結果 Array の monomorph を `recordArrayMonomorph` 経由で登録、call サイトで fn typedef を吐く順序を担保。
        - [ ] **`arrow_in_container_fail.ts` を positive 化** → `array_of_fn.ts` に rename + 内容を expand(reject 仕様の更新は `tests/smoke.sh` の `run_fail_case` 削除 + `run_case` 追加)。新規 fail は「Map / Set に fn を入れる」を残す(`Map<scalar, fn>` / `Set<fn>` は scope 外、必要なら別 step)。
        - [ ] **回帰**: `array_of_fn.ts`(arrow を `push` で詰める / `[i]` で読み出して `dispatch` / 別 arrow で `[i] = ...` 書き換え / 空 `Array<fn>` の `.push` / `.length` / `.map` で fn-を-返す-callback の結果を Array に貯める / for-of で arrow を順次呼び出す / Array<fn> を関数引数で受ける の 6-8 ケース)、fail 2 本(`map_of_fn_fail.ts` / `set_of_fn_fail.ts` で Map / Set に fn を入れる reject を確認、現行の `arrow_in_container_fail.ts` の `Map` / `Set` 版を切り出す)。
        - [ ] **scope 外(別 step に切り出し)**: `Map<scalar, fn>` / `Set<fn>`(value / 要素方向、Map / Set monomorph の生成器に fn elem を通す改修が要、必要になった時に別 step)。class / interface field に fn 型を持つ(class / interface struct に fn typedef を埋め込む構造変更が要、自己参照する fn 型と循環するので慎重に)。Array<fn> の elem を `.includes` で reference identity 比較(fn の reference identity は意味が曖昧、`arr.indexOf(f)` も含めて保留)。Array<fn> の `.join`(format policy 不明、保留)。
