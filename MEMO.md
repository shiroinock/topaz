...
- [ ] **1.5 残り** — 以下を 1.5-N サブフェーズに分けて進める。順序は self-hosting に必要なものから逆算で入れ替える。詳細な棚卸し根拠は `docs/archive/self-hosting-inventory.md`。
  ...
  - [x] **1.5-6 prep** — self-hosting 着手前の地ならし、19 サブステップ完了(access modifier no-op / void return / field initializer + auto-ctor / type alias / object literal & anonymous class / object destructuring / optional parameter & field / Array<dunion> / module-level const hoist / string method (charCodeAt + slice) / dunion-context object literal / String.fromCharCode / node:fs.readFileSync / recursive type alias (SCC + 2-phase fill) / `T | undefined` for T = dunion / global parseInt + parseFloat / equality without redundant outer parens / dunion 共通フィールドの narrow なし read / compound condition narrowing (`&&` / `||` の右オペランド))。決定ログは #1〜#15 が `docs/archive/implementation-log.md`、#16 以降が `docs/adr/`(parseInt/parseFloat = `0003`、equality = `0004`、dunion 共通フィールド = `0005`、compound narrowing = `0006`)。次 blocker は `topaz_parser.ts:137` の `if (t.kind !== "punct" || t.op !== op) { throw ... }` 後の carry narrowing(compound 条件の両オペランドを De Morgan 結合する early-exit narrowing、1.5-3d で未対応と明記済み)。
