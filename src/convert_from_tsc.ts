...
class Converter {
  ...
  convertModule(): SourceModule {
    ...
    return {
      filePath: this.sf.fileName,
      isInternalModule: false,
      stableModuleId: "",
      lineStarts: [...this.sf.getLineStarts()],
      items,
    };
  }
  ...
  convertTypeParams(tps: ts.NodeArray<ts.TypeParameterDeclaration> | undefined): TypeParam[] {
    ...
    for (const tp of tps) {
      ...
      const defaultType = tp.default !== undefined ? this.convertType(tp.default) : undefined;
      out.push({ name: tp.name.text, constraint, defaultType, ...this.span(tp) });
    }
    return out;
  }
