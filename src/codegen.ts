...
type AwaitReturnInfo = {
  ...
  tempName: string;
  deferStatementCompletion: boolean;
};
...
type AwaitMaterializedTemp = {
  ...
  elemType?: TopazType;
  value?: Expr;
  receiverCapturedBeforeAwait: boolean;
  oldValueCapturedBeforeAwait: boolean;
  exprType: TopazType;
  pos?: number;
};
...
type MultiAwaitCallArgStepPlan = {
  awaitExpr: AwaitExpr;
  operandInfo: AwaitOperandInfo;
  awaitedType: TopazType;
  tempName: string;
  preAwaitReceiverTemps: Array<AwaitCallReceiverTemp>;
  preAwaitIndexTemps: Array<AwaitIndexTemp>;
...
class Emitter {
          ...
          if (init.kind === "await_expr") {
            ...
            if (initializerAwaits.length > 0) {
              ...
              if (initializerAwaits.length > 1) {
                ...
                const multiAwait =
                  this.tryBuildMultiAwaitAssignmentExpression(
                    initMaybe,
                ...
                let bindingType = this.inferTypeWithOptionalExpected(multiAwait.transformedExpr, expectedInitializerType);
        ...
        if (valueMaybe !== undefined) {
          ...
          if (returnAwaits.length > 0) {
            if (returnAwaits.length > 1) {
              ...
              const returnType = this.inferTypeWithOptionalExpected(multiAwait.transformedExpr, payloadType);
  ...
  private tryBuildAssignmentTargetRef(
    ...
    if (target.kind === "prop_access") {
      ...
      let fieldType: TopazType | undefined = undefined;
      if (isClassType(receiverType)) {
        ...
        fieldType = cls.fields.get(target.name);
        ...
        fieldType = iface.fields.get(target.name);
      } else {
        return undefined;
      }
    ...
    if (target.kind === "elem_access") {
      ...
      const indexTemp: AwaitIndexTemp = {
  ...
  private buildAwaitedAssignmentLeafWithValue(
    ...
    if (expr.op === "=" && targetRef.kind === "class_field") {
      ...
      this.assertNotVoid(targetRef.fieldType, { pos: expr.pos }, "await call-argument assignment leaf");
      return {
        kind: "class_field_assignment",
        targetRef,
        value: transformedValue,
        exprType: targetRef.fieldType,
        pos: expr.pos,
      };
    }

    if (expr.op === "=" && targetRef.kind === "interface_field") {
      this.expectType(transformedValue, targetRef.fieldType);
      this.assertNotVoid(targetRef.fieldType, { pos: expr.pos }, "await call-argument assignment leaf");
      return {
        kind: "interface_field_assignment",
    ...
    if (expr.op === "=" && targetRef.kind === "array_element") {
      this.expectType(transformedValue, targetRef.elemType);
      this.assertNotVoid(targetRef.elemType, { pos: expr.pos }, "await call-argument assignment leaf");
      return {
        kind: "array_element_assignment",
        targetRef,
        value: transformedValue,
        exprType: targetRef.elemType,
        pos: expr.pos,
      };
    }
    ...
    if (expr.op === "=") {
      return { kind: "expr", expr: transformedExpr, exprType, targetRef };
    }
    ...
    switch (materialized.kind) {
      case "identifier_compound_assignment":
        return {
          kind: "identifier_compound_assignment",
          tempName,
          receiverCapturedBeforeAwait: false,
          oldValueCapturedBeforeAwait: false,
          targetName: materialized.targetRef.targetName,
      ...
      case "class_field_compound_assignment":
        return {
          ...
          receiverCapturedBeforeAwait: false,
          oldValueCapturedBeforeAwait: false,
          receiverTempName: materialized.targetRef.receiverTemp.tempName,
          receiverType: materialized.targetRef.receiverType,
          fieldName: materialized.targetRef.fieldName,
          fieldType: materialized.targetRef.fieldType,
          oldValueTempName: materialized.oldValueTempName,
          op: materialized.op,
          value: materialized.value,
          exprType: materialized.exprType,
      ...
      case "array_element_assignment":
        return {
          ...
          oldValueCapturedBeforeAwait: false,
          receiverTempName: materialized.targetRef.receiverTemp.tempName,
          indexTempName: materialized.targetRef.indexTemp.tempName,
          arrayType: materialized.targetRef.arrayType,
          elemType: materialized.targetRef.elemType,
          value: materialized.value,
          exprType: materialized.exprType,
          pos: materialized.pos,
        };
  ...
  private tryBuildNestedMultiAwaitCallArgPlan(
    ...
    const nestedPlan: MultiAwaitNestedCallArgPlan = {
      argIndex: outerArgIndex,
      root: nestedRoot,
      resultTempName: `${tempPrefix}_nested_${nestedIndex}`,
      resultTempExpr: {
        kind: "ident",
        name: `${tempPrefix}_nested_${nestedIndex}`,
        pos: nestedRoot.pos,
        end: nestedRoot.end,
      },
      awaitedArgIndexes: [],
    ...
    for (let nestedArgIndex = 0; nestedArgIndex < nestedRoot.args.length; nestedArgIndex++) {
      ...
      if (unwrapped.kind === "call_expr") {
        ...
        nestedPlan.transformedNestedCallArgs.set(nestedArgIndex, childPlan.resultTempExpr);
        const firstChildAwaitIndex = childPlan.awaitedArgIndexes[0];
        nestedPlan.awaitedArgIndexes.push(firstChildAwaitIndex);
        nestedPlan.awaitedArgDependencies.push({ awaitIndex: firstChildAwaitIndex, argIndex: nestedArgIndex });
        callArgEvents.push({ kind: "materialize", argIndex: outerArgIndex, nestedIndex: childNestedIndex });
    ...
    if (nestedPlan.awaitedArgIndexes.length === 0) {
      return undefined;
    }

    nestedPlan.transformedBinaryArgs = transformedBinaryArgs;
    return nestedPlan;
  }
  ...
  private tryBuildMultiAwaitCallArgExpression(
    ...
    for (let i = 0; i < awaitedArgs.length; i++) {
      ...
      for (let argIndex = nextPreArgStart; argIndex < awaited.argIndex; argIndex++) {
        const nestedTemp = nestedResultByArg.get(argIndex);
        let arg: Expr = root.args[argIndex];
        if (nestedTemp !== undefined) arg = nestedTemp;
        ...
        const argType = param.type;
        this.assertNotVoid(argType, { pos: arg.pos }, "await call pre-argument temporary");
        const tempName = `__topaz_call_arg_${stepOrdinalStart}_${i}_${argIndex}`;
        this.scope.declareBinding(tempName, argType, /* isConst */ true, { pos: arg.pos });
        stepPlan.preAwaitArgTemps.push({ tempName, arg, argType });
        argTempByArg.set(argIndex, { kind: "ident", name: tempName, pos: arg.pos, end: arg.end });
      }
      nextPreArgStart = awaited.argIndex + 1;
    }
    ...
    for (let i = 0; i < root.args.length; i++) {
      ...
      if (argTemp !== undefined) transformedArgs.push(argTemp);
      else transformedArgs.push(root.args[i]);
    }
  ...
  private tryBuildMultiAwaitAssignmentExpression(
    expr: Expr,
    tempPrefix: string,
  ): MultiAwaitCallArgPlan | undefined {
    const root = this.unwrapParenExpr(expr);
    ...
    const events: Array<MultiAwaitLeafEvent> = [];
    if (!this.collectMultiAwaitBinaryLeafEvents(root.value, events)) return undefined;
    const awaitCount = events.filter((event) => event.kind === "await").length;
    if (awaitCount < 2) return undefined;

    let transformedValue: Expr = root.value;
    ...
    for (const event of events) {
      switch (event.kind) {
        ...
        case "await": {
          ...
          transformedValue = this.replaceAwaitExprInExpr(transformedValue, awaitExpr, tempExpr);
    ...
    const materializedTempName = `${tempPrefix}_assign_value`;
    this.scope.declareBinding(materializedTempName, materialized.exprType, /* isConst */ true, { pos: root.pos });
    const materializedTemp = this.buildAwaitMaterializedTemp(materializedTempName, materialized);
    materializedTemp.receiverCapturedBeforeAwait = true;
    if (root.op !== "=") materializedTemp.oldValueCapturedBeforeAwait = true;
    steps[steps.length - 1].postAwaitMaterializedTemps.push(materializedTemp);

    const transformedExpr: IdentExpr = {
      kind: "ident",
      name: materializedTempName,
      pos: root.pos,
      end: root.end,
          ...
          for (const prop of expr.props) {
            if (prop.kind === "prop_kv") {
              props.push({
                kind: "prop_kv",
                name: prop.name,
                value: this.replaceExactExprInExpr(prop.value, target, replacement),
                pos: prop.pos,
                end: prop.end,
              });
            } else if (prop.kind === "prop_shorthand") {
              ...
              props.push({
                kind: "prop_spread",
                expr: this.replaceExactExprInExpr(prop.expr, target, replacement),
                pos: prop.pos,
                end: prop.end,
              });
            }
  ...
  private addAwaitMaterializedTempFrameFields(fields: string[], temp: AwaitMaterializedTemp): void {
    if (
      ...
      if (temp.oldValueCapturedBeforeAwait !== true) {
  ...
  private emitAwaitMaterializedTempPreAwaitStores(
    ...
    for (const temp of temps) {
      ...
      if (temp.oldValueCapturedBeforeAwait === true) continue;
      const oldValueTempName = temp.oldValueTempName;
      if (oldValueTempName === undefined) {
        throwInternalCodegenError("compound assignment materialized temp missing old-value temp");
      }
  ...
  private emitAwaitSnapshotTempRestores(
    ...
    for (const temp of temps) {
      ...
      if (
        ...
        const target = this.unwrapParenExpr(snapshotExpr.operand);
        if (target.kind === "ident") {
          const delta = snapshotExpr.op === "++" ? "+ 1.0" : "- 1.0";
          lines.push(`${indent}${target.name} = ${temp.tempName} ${delta};`);
        }
      }
  ...
  private awaitMaterializedTempPos(temp: AwaitMaterializedTemp): number {
    ...
    if (temp.kind === "class_field_assignment") {
      const pos = temp.pos;
      if (pos === undefined) throwInternalCodegenError("class assignment materialized temp missing position");
      return pos;
    }
  ...
  private emitAwaitMaterializedTempExpression(temp: AwaitMaterializedTemp, frameRef: string): string {
    ...
    if (temp.kind === "class_field_compound_assignment") {
      ...
      const valueTmp = `${temp.tempName}_value`;
      const nextTmp = `${temp.tempName}_next`;
      const receiverExpr = temp.receiverCapturedBeforeAwait === true ? `${frameRef}->${receiverTempName}` : receiverTempName;
      ...
      return `({ ${cTypeName(fieldType)} ${valueTmp} = ${valueExpr}; ${cTypeName(fieldType)} ${nextTmp} = ${nextExpr}; ${receiverExpr}->${fieldName} = ${nextTmp}; ${nextTmp}; })`;
    }
    if (temp.kind === "interface_field_compound_assignment") {
      const fieldType = temp.fieldType;
      const value = temp.value;
      ...
      const oldValueExpr = `${frameRef}->${oldValueTempName}`;
      const nextExpr = this.emitCompoundAssignmentNextExpression(op, oldValueExpr, valueTmp, fieldType, { pos });
      return `({ ${cTypeName(fieldType)} ${valueTmp} = ${valueExpr}; ${cTypeName(fieldType)} ${nextTmp} = ${nextExpr}; ${receiverExpr}.vt->set_${fieldName}(${receiverExpr}.data, ${nextTmp}); ${nextTmp}; })`;
    }
    if (temp.kind === "class_field_assignment") {
      const fieldType = temp.fieldType;
      const value = temp.value;
      const receiverTempName = temp.receiverTempName;
      const fieldName = temp.fieldName;
      if (fieldType === undefined) throwInternalCodegenError("class assignment materialized temp missing field type");
      if (value === undefined) throwInternalCodegenError("class assignment materialized temp missing value");
      if (receiverTempName === undefined) throwInternalCodegenError("class assignment materialized temp missing receiver temp");
      if (fieldName === undefined) throwInternalCodegenError("class assignment materialized temp missing field name");
      const receiverExpr = temp.receiverCapturedBeforeAwait === true ? `${frameRef}->${receiverTempName}` : receiverTempName;
      const valueTmp = `${temp.tempName}_value`;
      const valueExpr = this.emitWithExpected(value, fieldType);
      return `({ ${cTypeName(fieldType)} ${valueTmp} = ${valueExpr}; ${receiverExpr}->${fieldName} = ${valueTmp}; ${valueTmp}; })`;
    }
    if (temp.kind === "interface_field_assignment") {
      const fieldType = temp.fieldType;
      const value = temp.value;
      ...
      const valueTmp = `${temp.tempName}_value`;
      const valueExpr = this.emitWithExpected(value, fieldType);
      return `({ ${cTypeName(fieldType)} ${valueTmp} = ${valueExpr}; ${receiverExpr}.vt->set_${fieldName}(${receiverExpr}.data, ${valueTmp}); ${valueTmp}; })`;
    }
    if (temp.kind === "array_element_assignment") {
      ...
      const receiverExpr = temp.receiverCapturedBeforeAwait === true ? `${frameRef}->${receiverTempName}` : receiverTempName;
      const indexExpr = temp.receiverCapturedBeforeAwait === true ? `${frameRef}->${indexTempName}` : indexTempName;
      const valueTmp = `${temp.tempName}_value`;
      const valueExpr = this.emitWithExpected(value, elemType);
      const arrayName = arrayShortName(arrayType);
      return `({ ${cTypeName(elemType)} ${valueTmp} = ${valueExpr}; topaz_array_${arrayName}_set(${receiverExpr}, ${indexExpr}, ${valueTmp}); ${valueTmp}; })`;
    }
    if (temp.kind === "array_element_compound_assignment") {
      const elemType = temp.elemType;
      const arrayType = temp.arrayType;
      ...
      return `({ ${cTypeName(elemType)} ${valueTmp} = ${valueExpr}; ${cTypeName(elemType)} ${nextTmp} = ${nextExpr}; topaz_array_${arrayName}_set(${receiverExpr}, ${indexExpr}, ${nextTmp}); ${nextTmp}; })`;
    }
    const expr = temp.expr;
    if (expr === undefined) throwInternalCodegenError("expression materialized temp missing expression");
  ...
  private asyncStepDefersStatementCompletion(step: AsyncSuspensionStep): boolean {
    switch (step.kind) {
      case "binding":
        return false;
      case "initializer":
        return step.deferStatementCompletion === true;
      case "return":
        return step.deferStatementCompletion === true;
      case "statement":
        return step.deferStatementCompletion === true;
  ...
  private emitAsyncStepTempRestores(
    step: AsyncSuspensionStep,
    lines: string[],
    indent: string,
  ...
  private recordAsyncAwaitRunner(
    ...
    for (const binding of frame.steps) {
      if (binding.kind === "binding") {
        ...
        if (binding.transformedExpr !== undefined || this.asyncStepDefersStatementCompletion(binding)) {
          for (const receiverTemp of binding.preAwaitReceiverTemps) {
            fields.push(`  ${cTypeName(receiverTemp.receiverType)} ${receiverTemp.tempName};`);
          }
          for (const indexTemp of binding.preAwaitIndexTemps) {
