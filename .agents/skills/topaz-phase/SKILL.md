...
まず `AGENTS.md` を読んで現状仕様の固定点(divergence・作業ルール)を確認する。`MEMO.md` の現行ロードマップで残タスクのうちどこを着地させるかを決める。
...
- **既存機能の divergence 解消 / 仕様強化**: `AGENTS.md` の「既知の divergence」または fail サンプルから 1 つ
...
長さの目安は **30〜50 行**。MEMO.md / AGENTS.md には書かない(凍結 archive である `docs/archive/implementation-log.md` にも追記しない)。
...
AGENTS.md はサブステップの進捗管理を持たない方針なので原則触らない。divergence / 作業ルールが本質的に変わった時のみ。
