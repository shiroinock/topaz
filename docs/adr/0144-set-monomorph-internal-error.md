# 0144. set monomorph internal error (6i prep)
...
[0143](./0143-set-monomorph-hash-eq-initializers.md) moved the full graph
self-host probe to `src/codegen.ts:2866`, where `emitSetMonomorphMacro` used
`throw new Error(...)` for an unsupported set element kind. Topaz does not
...
- `src/codegen.ts:2866` calls `throwInternalCodegenError` with the existing
  diagnostic text.

## Consequences

- **Accepted**: set monomorph impossible states use the compiler internal error
  channel.
