...
- **Phase 1.4** — 1.4a で class(継承・ジェネリクスなし、明示 constructor、reference 型、`this` / プロパティ read/write / メソッド呼び出し / `new ClassName(args)`、done)、1.4b で interface(`implements` 宣言、fat pointer + per-(I, C) vtable、EXACT 構造的一致、class→interface coercion は 4 サイト限定、done)、1.4c で generics(関数・クラスの型パラメータ、monomorphize、`Array<ClassName>` / `Map<K, ClassName>` / `Array<InterfaceName>` 等のコンテナ展開もこのタイミング)。
