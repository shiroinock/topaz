...
**現在のステータス**: Phase 1.4c-2 完了(generic function、`function f<T>(...)` を worklist monomorphize で着地)。**次は Phase 1.4c-3(generic class、`class Box<T> { ... }`)** + 同タイミングで `TopazType` を structured 表現へ移行予定。残タスクの全体像は `MEMO.md §6` を参照。
...
- **class の未対応リスト(明示エラー)**。`extends`、`static` / `private` / `protected` / `public` / `readonly` / `abstract` / `override` 修飾子、generic class / generic method(Phase 1.4c-3 以降)、class expression、decorator、getter / setter、static block、parameter property shorthand(`constructor(public x: number)`)、`super`、暗黙コンストラクタ、メソッド参照(`obj.method` without `()`)、constructor 内 `return`、class を `console.log` の引数に取る、`Map<ClassName, V>` で class を key にする方向(Map / Set / Array の値 / 要素方向は対応済み)。
...
- **Phase 1.4c-3** — generic class(`class Box<T> { ... }`、`new Box<number>()` でインスタンス化、コンテナ自前定義が可能になる)。値型コンテナを書けるようになるので、Set の構造的等値の必要性を再評価する。同タイミングで `TopazType` を string union から structured 表現(`{ kind: "array", elem: TopazType }` 等)へ移行する(`Array<Box<number>>` のようなネストが入るため)。
